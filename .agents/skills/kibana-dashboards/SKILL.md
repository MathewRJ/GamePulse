...
## GamePulse-Specific Lessons (Elastic Cloud Serverless)

These lessons were validated against Elastic Cloud Serverless (ES 8.13 / Kibana 9.4) in
the GamePulse project. Apply them any time you work in a Serverless context.

### Field Path Rules: `.keyword` vs Native Keyword

ES index templates evolve over time — the right field path depends on when the backing
index was created:

- **Old backing indices** (created before index template was deployed): text fields have a
  `.keyword` sub-field (e.g., `gamepulse.game.name.keyword`). Use `.keyword` for
  aggregations and filter controls.
- **New backing indices** (created after index template): fields declared as `keyword` are
  native keyword with NO `.keyword` sub-field. Use the bare path
  (e.g., `gamepulse.game.name`). Using `.keyword` on these returns empty results.

**Rule:** Always validate field paths against the current backing index before building
panels. Use an ES|QL query to confirm the field exists and has the expected type:
```esql
FROM metrics-gamepulse.session-default
| KEEP gamepulse.game.name
| LIMIT 1
```
If `verification_exception` appears, there are multiple backing indices with incompatible
types — see the backing index conflict section below.

**`options_list_control` filter**: filter control `field_name` MUST use `.keyword` for any
text-with-keyword-subfield. Bare text fields silently produce a non-functional filter.
OS filter: use `host.os.name.keyword`.

**`last_value` in data tables**: `sourceField` for text fields also needs `.keyword`
(e.g., `host.os.kernel.keyword`, `gamepulse.hardware.gpu.model.keyword`).

### Backing Index Type Conflicts

When a data stream has multiple backing indices with incompatible field type mappings
(e.g., `text` in index 000001, `keyword` in index 000002), ES|QL returns:
```
verification_exception: Found N problems: field_name: keyword in 000002, text in 000001
```

**Fix:** Delete old backing indices that pre-date the correct index template deployment.
**TSDS backing indices cannot be reindexed** — documents timestamped outside the new
backing index's writable window are rejected with `timestamp_error`.

**Prevention:** Always deploy the integration package (index templates) BEFORE collecting
live data.

### TSDS: Dimension Fields and Nested Types

In `data_stream/*/manifest.yml`, `index_mode: "time_series"` (TSDS mode) requires:
- `dimension: true` on keyword fields used as TSDS dimensions
- No `object` or `nested` field types (incompatible with synthetic source on ES 8.13)

For data streams that have nested fields (e.g., `thread_breakdown nested`), remove
`index_mode: "time_series"` and all `dimension: true` annotations — use a regular
metrics data stream instead.

`elastic-package check` will catch these as errors if misconfigured.

### Saved Objects Import API (Serverless)

The newer Kibana Dashboards API (9.4+, this skill's primary workflow) is not always
available or convenient. The `_import` fallback works reliably on Serverless:

- `POST /api/saved_objects/_import?overwrite=true` with `Content-Type: multipart/form-data`
- File MUST have `.ndjson` extension (`.json` returns HTTP 400 "Invalid file extension")
- Auth: use `ES_API_KEY` (not a separate KIBANA_API_KEY — same key works for both ES and Kibana on Serverless)
- `GET /api/saved_objects/dashboard/{id}` returns 400 on Serverless ("not available")
- Use `POST /api/saved_objects/_export` with body `{"objects":[{"type":"dashboard","id":"..."}],"includeReferencesDeep":false}` to fetch an existing dashboard

### Panel Structure: By-Value Panels

Every Lens panel must embed its visualization definition inline in `embeddableConfig.attributes`
(by-value). Do NOT use `panelRefName` or `savedObjectId` references for panels — these
require a separate saved object and break portability.

When fixing or updating a dashboard fetched via `_export`:
- The NDJSON line contains the full dashboard saved object
- `embeddableConfig.attributes` holds the complete Lens definition
- Columns in `formBased.layers.<id>.columns` are referenced by `columnOrder` (list) and
  `visualization.columns` (display metadata). A missing field (e.g., `proton_version` not
...
these fields. Do NOT reference fields not in this list — missing fields cause panel render errors:

| Field | Type | Notes |
|-------|------|-------|
| `game_name` | keyword | |
| `session_id` | keyword | |
| `session_start` | date | time field for data view |
| `avg_fps` | float | |
| `low_1pct_fps` | float | |
...
| `stutter_count` | long | |
| `total_frames` | long | |
| `cumulative_playtime_hours` | float | computed by Python post-enrichment |

**Not present:** `proton_version`, `session_label` — neither written to this index.
Using these as `sourceField` in a panel causes a render error on the whole panel.
