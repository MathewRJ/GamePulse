...
_installer() {
  local -a args=(--bundle "$BUNDLE" --endpoint "$ES_URL" --ca-file "$CS_CA_FILE" --kibana-endpoint "$KB_URL" --kibana-ca-file "$CS_CA_FILE" --admin-credentials-file "$RUN_DIR/admin.toml" --agent-binary "$CLEAN_STACK_AGENT_BINARY" --profile user --enrollment-root "$RUN_DIR/enrollment" --ownership-profile fleet-coexist --adopt-existing-w1-stream)
...
leg_d() { setup; installer; expect_refusal omitted_profile_on_coexist env -u RIGSIGNAL_HTTP_AUDIT_LOG python3 "$REPO_ROOT/tools/install_assets.py" --bundle "$BUNDLE" --endpoint "$ES_URL" --ca-file "$CS_CA_FILE" --kibana-endpoint "$KB_URL" --kibana-ca-file "$CS_CA_FILE" --admin-credentials-file "$RUN_DIR/admin.toml" --agent-binary "$CLEAN_STACK_AGENT_BINARY" --profile user --enrollment-root "$RUN_DIR/enrollment" --adopt-existing-w1-stream; expect_refusal ownership_profile_mismatch env python3 "$REPO_ROOT/tools/install_assets.py" --bundle "$BUNDLE" --endpoint "$ES_URL" --ca-file "$CS_CA_FILE" --kibana-endpoint "$KB_URL" --kibana-ca-file "$CS_CA_FILE" --admin-credentials-file "$RUN_DIR/admin.toml" --agent-binary "$CLEAN_STACK_AGENT_BINARY" --profile user --enrollment-root "$RUN_DIR/enrollment" --ownership-profile default --adopt-existing-w1-stream; expect_refusal ownership_table_unresolved installer_unresolved; expect_refusal cluster_health installer_bad_health; expect_refusal ilm_delete_phase installer_bad_ilm; printf '[elasticsearch]\napi_key = "x"\n' >"$RUN_DIR/api-key.toml"; chmod 600 "$RUN_DIR/api-key.toml"; expect_refusal admin_credential_api_key env CLEAN_STACK_INSTALLER="$REPO_ROOT/tools/install_assets.py" python3 "$REPO_ROOT/tools/install_assets.py" --bundle "$BUNDLE" --endpoint "$ES_URL" --ca-file "$CS_CA_FILE" --kibana-endpoint "$KB_URL" --kibana-ca-file "$CS_CA_FILE" --admin-credentials-file "$RUN_DIR/api-key.toml" --agent-binary "$CLEAN_STACK_AGENT_BINARY" --profile user --enrollment-root "$RUN_DIR/api-key-root" --ownership-profile fleet-coexist --adopt-existing-w1-stream; }
