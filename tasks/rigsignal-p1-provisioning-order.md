...
| enrollment state binding | A present `state.json` lacks a valid `enrollment_root`, or its value differs from the canonical actual enrollment root. | installer nonzero: `install refused: enrollment state is not valid for this enrollment root`; purge nonzero: `uninstall purge failed: enrollment state validation:` | Refuse before any HTTP request or local mutation: no stored ID/name consumption, key discovery/mint/invalidation, enrollment/marker write or advance, or committed/candidate deletion; cluster and local owned-state snapshots remain equal. |
...
state.json is duplicate-key-rejecting JSON whose only allowed keys are version, phase, expected_cluster_uuid, target_generation, role_jcs_sha256, enrollment_root, active_key_id, pending_revoke_ids, pending_mint_name, and candidate_key_id; all ten are required (root-binding AMENDED 2026-07-23d, Sol-sparred, owner-ratified; version remains 1 because no prior state has shipped). enrollment_root is a non-empty, absolute, strict-UTF-8 path string containing no NUL and at most 4096 UTF-8 bytes: the canonical actual enrollment root selected for the invocation (production default or test-only --enrollment-root), computed with os.path.realpath after the directory exists and passes the v2.3 invoking-UID/0700/no-follow checks. A symlink anywhere in the enrollment-root path is rejected; canonicalization does not make symlink traversal acceptable. The canonical value is recorded before the first state write and copied unchanged through every phase, candidate state, and atomic publication; a noncanonical enrollment root rejects. version is integer 1; phase is one of committed, mint_intent, candidate_staged, candidate_verified; expected_cluster_uuid is a 22-character base64url string; target_generation and role_jcs_sha256 are 64-lowercase-hex strings; active_key_id, pending_mint_name, and candidate_key_id are string-or-null; pending_revoke_ids is an array of sorted, unique strings. Every key ID is at most 1024 bytes and pending_mint_name at most 255 bytes; unknown keys, wrong types, invalid nullability, duplicate IDs, or invalid encodings reject.
...
Every state read that can establish installation ownership or supply identifiers for key discovery, invalidation, or deletion — including installer recovery, v2.2 fence bullet 2, rerun, and uninstall --purge/purge_ids — must validate state.enrollment_root == os.path.realpath(actual_enrollment_root) BEFORE any HTTP request, key lookup/discovery/mint/invalidation, or local write/delete. Missing, invalid, or mismatched binding refuses; neither installer nor purge may rebind or repair the state, consume any stored key ID or mint name, revoke any key, remove candidate or committed material, or write/advance credentials, configuration, capsule, state, or marker. For v2.2 bullet 2, "belongs to this installation" includes this successful root-binding check. The test-only --enrollment-root is the actual root in use, so test roots bind to their own canonical paths. Scope: this is an anti-misassociation guard, not cryptographic host identity — the same absolute path on another host or mount namespace remains indistinguishable.
...
|---|---|---|
| positive/24-event-bytes-exact-cap.input.json and .expected.json | Literal expansion of positive/01-diagnosis.input.json with only plain_language replaced by ASCII a repeated exactly 1,047,210 times; its RFC-8785 event is exactly 1,048,576 bytes (canonical JCS bytes, excluding the storage file's terminal LF per MANIFEST.md). | Exactly {"result":"accepted","serialized_bytes":1048576}. |
| negative/24-event-bytes-one-over.input.json and .expected.json | Literal expansion of the same source with plain_language ASCII a repeated exactly 1,047,211 times; serialized length exactly 1,048,577. | Exactly {"variant":"EventBytesLimitExceeded","limit":1048576,"actual_saturated":1048577}. |
| positive/25-event-bytes-unicode-escape.input.json and .expected.json | Literal expansion of same base with plain_language equal to (U+00E9 followed by U+000A) repeated exactly 8,192 times, then ASCII a repeated exactly 1,014,442 times. Its source is 1,039,018 UTF-8 bytes while its JCS string is 1,047,210 bytes and event is 1,048,576 bytes. | Exactly {"result":"accepted","serialized_bytes":1048576,"source_plain_language_utf8_bytes":1039018}. |
