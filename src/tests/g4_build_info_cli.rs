...
fn assert_build_info(output: Output, name: &str) {
    assert!(
...
#[test]
fn build_info_is_machine_readable_and_version_remains_available() {
    assert_build_info(
        agent()
            .arg("--build-info-json")
            .output()
            .expect("run rigsignal-agent --build-info-json"),
        "rigsignal-agent",
    );
    ...
    let output = agent()
        .arg("--version")
        .output()
        .expect("run rigsignal-agent --version");
