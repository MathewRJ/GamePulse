...
That's it. GamePulse starts automatically when you launch the game and stops
...
and write `~/.config/gamepulse/gamepulse.toml`. Your API key is stored only in that
...
2. Create a key with write access to `metrics-gamepulse.*` and `logs-gamepulse.*`
...
```sh
gamepulse start   # before launching a game
gamepulse stop    # after exiting
```

`gamepulse start` starts the metrics agent (always) and the eBPF kernel daemon
...
`gamepulse run %command%` does three things:

1. **Starts** the gamepulse-agent (user service) and gamepulse-ebpf daemon
...
**"gamepulse: command not found"**
The AUR package installs `gamepulse` to `/usr/bin/gamepulse`. If you installed
from source, ensure `packaging/gamepulse-launcher.sh` is on your PATH.

**"Failed to start gamepulse-agent"**
Check that the systemd user service is installed:
```sh
systemctl --user status gamepulse-agent
```
If not found, install the gamepulse AUR package or copy the service unit manually:
```sh
install -Dm644 packaging/systemd/gamepulse-agent.service \
...
permission for your user. Without it, GamePulse runs in agent-only mode —
...
```
# /etc/sudoers.d/gamepulse-ebpf
%wheel ALL=(ALL) NOPASSWD: /usr/bin/systemctl start gamepulse-ebpf
%wheel ALL=(ALL) NOPASSWD: /usr/bin/systemctl stop gamepulse-ebpf
```
...
```sh
journalctl --user -u gamepulse-agent -n 50 | grep "Game detected"
```
If not present, the game may have launched before the agent started (the agent
needs ~5 seconds to begin scanning). This is unlikely with `gamepulse run` since
