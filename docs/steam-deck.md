...
RigSignal is optimised for the Steam Deck. It automatically detects the device model (LCD vs OLED), reads the APU sensors via sysfs, and integrates with gamescope for frame timing.
...
This installs `rigsignal-agent` and the `rigsignal` launcher to `~/.local/bin/`. If the release
includes the eBPF daemon, the installer will prompt for `sudo` to place it in `/usr/local/bin/`
and enable `rigsignal-ebpf.service` immediately — kernel tracing is active without any follow-up
...
Optional settings you can add manually to `~/.config/rigsignal/rigsignal.toml`:
...
daemon (`rigsignal-ebpf.service`) is enabled and started by the installer — no separate step needed.

## Storage metrics on Steam Deck

RigSignal classifies and monitors both the internal NVMe and SD card:
...
When you have games installed on both drives, RigSignal detects which drive the currently running game is installed on and focuses I/O monitoring there. The session document records both drives for comparison.
...
1. **Gamescope stats** — gamescope (the Steam Deck compositor) exposes frame timing data. RigSignal reads this when running in Gaming Mode.
...
RigSignal reads the Steam Deck's power envelope (TDP) from sysfs. Combined with FPS data, this enables performance-per-watt analysis — answering questions like "what TDP setting gives me stable 40fps in this game?"

## What gets detected automatically

When you launch a game on Steam Deck, RigSignal automatically captures:
...
- **eBPF requires root**: the installer handles this automatically (prompts for `sudo` and enables `rigsignal-ebpf.service`). If eBPF was skipped during install, re-run the installer with `sudo` access to enable kernel tracing.
...
- **Gaming Mode limitations**: the agent runs fine in Gaming Mode, but you can't see debug output. Check logs with `journalctl --user -u rigsignal-agent` in Desktop Mode.
