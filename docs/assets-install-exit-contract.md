...
| `enrollment_remediation_required` | 3 | If the parent contains an effective-user-owned `0700` `.rigsignal-publication-<root-name>` or `.rigsignal-publication-<root-name>-<16-lowercase-hex>` directory, inspect it and remove only that exact owned private debris. Never delete a foreign or insecure lookalike. | `root_prepare`. |
