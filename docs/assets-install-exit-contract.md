...
| `enrollment_remediation_required` | 3 | If the parent contains an effective-user-owned `0700` `.rigsignal-publication-<root-name>` or `.rigsignal-publication-<root-name>-<16-lowercase-hex>` directory, inspect it and remove only that exact owned private debris. Never delete a foreign or insecure lookalike. The debris may hold an UNCOMMITTED minted credential: remove only the debris directory itself — never the root's `candidate/` record or `state.json`, whose recorded mint is what the rerun's recovery revokes. Separate `.rigsignal-publication-probe-*` directories are inert capability-probe litter from an abrupt earlier death; they authorize nothing and may be removed at leisure. | `root_prepare`. |
| `enrollment ancestor is not protected: *`, `outbox preflight: *`, `enrollment preflight unavailable`, `atomic_publication_filesystem_unsupported`, `enrollment_publication_path_too_long`, `enrollment_parent_fsync_unsupported`, `local_transaction_storage_unavailable`, `enrollment_ca_path_invalid` | 2 before apply; 4 if the current or durable recovery state indicates a possible mutation | Fix the local enrollment filesystem/CA/preflight condition, then use full-flow recovery where applicable. The real exchange probe reports `atomic_publication_filesystem_unsupported` at `preflight` normally and `root_prepare` on the incomplete path. | `root_prepare`/`candidate_stage`; exit derives from current `mutation_issued` plus durable `possible_mutation`. |
| `enrollment publication anchor is unavailable`, `enrollment publication identity changed`, `enrollment publication cleanup failed` | 2, 3, or 4 under the ordinary mutation contract | Inspect the enrollment parent and both generation names; do not assume an automatic rerun is safe. Identity failures intentionally suppress journal persistence through the disputed root. The generic catch-all emits `install failed: enrollment output:`; use the failure-site line to distinguish these cases. | `publication_anchor`, `publication_identity`, or `publication_cleanup`. |
...
Enrollment publication requires one installer instance per enrollment root.
The random stage name is not a publication mutex: two cooperating same-uid
installers against the same root are outside this contract and must be
serialized by their caller.  The remaining final-name race is therefore
bounded by the protected-parent check and this single-instance requirement.
